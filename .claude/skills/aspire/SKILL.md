...
**Important:** Use `~/.dotnet/tools/aspire` explicitly — Aspire 13.1.1 at `~/.aspire/bin/aspire` may shadow the 13.2 version on PATH.
...
```bash
~/.dotnet/tools/aspire start --isolated
~/.dotnet/tools/aspire wait myapi
```
...
1. `~/.dotnet/tools/aspire describe` — check resource status
2. `~/.dotnet/tools/aspire otel logs <resource>` — view structured logs
3. `~/.dotnet/tools/aspire logs <resource>` — view console output
4. `~/.dotnet/tools/aspire otel traces <resource>` — view distributed traces
...
```bash
~/.dotnet/tools/aspire mcp tools
~/.dotnet/tools/aspire mcp tools --format Json
~/.dotnet/tools/aspire mcp call <resource> <tool> --input '{"key":"value"}'
```
